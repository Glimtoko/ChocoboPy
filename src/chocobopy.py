...
if usehcmesh:
    ...
    ic.set_initial_conditions_sod(mesh)
else:
    ...
    # Read geometry
    rm.read_mesh(meshfile, inputs.boundary_values, inputs.material_list, mesh)
    
    # Set initial conditions
    ic.set_initial_conditions(eosfile, mesh)
...
for i in range(mesh.ncells):
    m = mesh.material[i]
    mesh.energy[i] = mesh.pressure[i]/((inputs.gamma[m] - 1.0)*mesh.ρ[i])
...
while time <= inputs.tend:
    ...
    # Set soundspeed
    hydro.set_soundspeed(mesh.pressure, mesh.ρ, mesh.soundspeed, mesh.material)

    # Calculate artificial viscosity
    hydro.get_q(mesh.ρ, mesh.soundspeed, mesh.fem.divv, mesh.area, mesh.q)

    # Calculate timestep
    dt, control = hydro.get_dt(mesh.area, mesh.soundspeed, mesh.ρ, mesh.q, dt, time)
    time += dt
    ...
    volume_old = copy.copy(mesh.volume)
    hydro.calculate_area_volume(
        mesh.coordinates05.x, mesh.coordinates05.y, mesh.nodelist,
    ...
    eos.perfect_gas(mesh.energy05, mesh.ρ05, mesh.pressure05, mesh.material)
    ...
    eos.perfect_gas(mesh.energy, mesh.ρ, mesh.pressure, mesh.material)
