...
def read_mesh(filename, material_list, mesh):
    ...
    with open(filename, "r") as mfile:
        for line in mfile:
            ...
            if innodelist:
                ...
                mesh.coordinates.x[index_node] = float(data[1])
                mesh.coordinates.y[index_node] = float(data[2])
            ...
            if incelllist:
                ...
                for i in range(4):
                    mesh.nodelist[index_cell,i] = n[i] - 1  # Correct to Python indexing
    ...
    for reg in range(1,nreg+1):
        ...
        if not canuserange:
            ...
            mesh.regioncelliterator[reg] = (first, last+1)
